...
class TestDay16(unittest.TestCase):
    ...
    def test_parser(self):
        ...
        for j, (expected, actual) in enumerate(zip(expected_output, parser.other_tickets)):
            self.assertEqual(expected, actual.is_valid(),
                             f"ticket {j} failed with matrix \n {actual._field_validation_matrix}")
...
class TestDay17(unittest.TestCase):
    ...
    def test_netherworld_creation(self):
        ...
        # Should have 5 nodes in the netherworld
        self.assertEqual(len(netherworld.nodes.keys()), 5)
        test_node = day17.Node(netherworld,(1,1,0), False)
        self.assertEqual(test_node.n_enabled_neighbors(), 5)
        # cycle and make sure that the expected nodes are updated
        netherworld.cycle()
        expected_nodes = ((0,1,0), (2,1,0), (0,1,-1), (0,1,1))
        for coord in expected_nodes:
            self.assertTrue(netherworld.nodes.get(coord, False),f"Coord {coord} does not exist in nodes")
        #print(len(netherworld.nodes))

        netherworld = day17.PocketNetherWorld(test_input_dir / "day17_test_input.txt")
        [netherworld.cycle() for j in range(6)]
        self.assertEqual(len(netherworld.nodes.keys()), 112)

    def test_4d_netherworld(self):
        netherworld = day17.PocketNetherWorld(test_input_dir / "day17_test_input.txt", 4)
